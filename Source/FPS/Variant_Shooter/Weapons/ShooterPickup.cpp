...
void AShooterPickup::OnOverlap(UPrimitiveComponent* OverlappedComponent, AActor* OtherActor, UPrimitiveComponent* OtherComp, int32 OtherBodyIndex, bool bFromSweep, const FHitResult& SweepResult)
{
	...
	if (!HasAuthority())
	{
		Server_RequestPickup(OtherActor);
		return;
	}
...
void AShooterPickup::Server_RequestPickup_Implementation(AActor* RequestingActor)
{
	TryPickup(RequestingActor);
}

void AShooterPickup::TryPickup(AActor* OtherActor)
{
